...
- Finals from Speechmatics are non-cumulative; multiple `AddTranscript` messages for an utterance each contain only the segment that just finalized. Do not assume finals prefix previously delivered text.
- ForceEndOfUtterance exists as a server message to request a finalized transcript; investigate using it (possibly paired with EndOfStream or a short trailing silence pad) to flush buffered text when user stops speaking without resuming.
